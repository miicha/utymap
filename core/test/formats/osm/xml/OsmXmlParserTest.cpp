...
namespace {
    ...
    {
        BOOST_CHECK_EQUAL(coordinate.latitude, node.coordinate.latitude);
        BOOST_CHECK_EQUAL(coordinate.longitude, node.coordinate.longitude);
...
BOOST_AUTO_TEST_CASE(GivenTestDummyXml_WhenParserParse_ThenHasProperNodes)
{
    ...
    OsmDataVisitor visitor(*dependencyProvider.getStringTable(), [&](Element& element) {
        if (Node* node = dynamic_cast<Node*>(&element)) {
            if (node->id == 42421951) {
                assertNode(*node, utymap::GeoCoordinate(40.7033242, -74.0077787), { createTag("highway","traffic_signals") });
                checkList[0] = true;
            }
